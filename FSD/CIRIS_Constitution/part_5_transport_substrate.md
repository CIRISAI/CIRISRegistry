...
Per + [ciris.ai/cewp](https://ciris.ai/cewp) load-bearing claim:
...
**Composition with self-attestation pattern**: the CEG-native agent's `consent:partnered:{user_key}` self-attestation (producer-side stance) MAY ride local-tier; the user's `consent:state:revoked` against the agent's stance Contribution (subject-side) MUST NOT. This preserves the cardinality wins from #840 while closing the leak window.
...
**Canonical bytes — `JCS(envelope)` per [CC 2.6.1](00_conformance.md) / RFC 8785:**
...
Per [ absorbed](https://github.com/CIRISAI/CIRISRegistry/issues/44) + (observer-share driver) + (streaming substrate prerequisite). CC 5.3.3 is the **delivery axis** — the third orthogonal envelope concern alongside visibility (`cohort_scope`) and revocability. [CC 2.4](03_primitives.md)'s 1+4 primitive set is untouched; CC 5.3.3 is endpoint + envelope + composition extension, NOT a grammar change.
...
- **Witness cosign — OPTIONAL**, via the [CC 5.3.1](#103-sth-cosigning--witness-directory) path verbatim. This is the best-effort / accountable split:
 ...
 - **Accountable** (paid media, registry propagation, emergency) → witness cosign per [CC 5.3.1.1](#1031-consistency-proof-requirement-normative-addresses-ceg-01-distsys-review) consistency-proof, which is the **anti-equivocation guarantee** — producer cannot show different chunk-K to different subscribers nor rewrite mid-stream. Accountable tier is impl-pending BOTH #142 AND (STH consistency-proof enforcement)
- **Cadence**: producer publishes a signed root every `K` chunks OR `T` seconds (whichever first), **always at an epoch boundary** (CC 5.1) + at `sealed_at`. Witness cosign runs **per-epoch** (coarser than per-K to keep cosign-quorum cost off the hot path). Default pins: **K=64, T=2s**
