...
- `takedown_notice{content_sha256}` → the content's `subject_key_ids` (self) ∪ `is_named_moderator(·, C, takedown)` for the content's community `C` (its `cohort_scope: community` / `community_id`).
