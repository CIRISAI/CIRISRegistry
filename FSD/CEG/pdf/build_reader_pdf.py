#!/usr/bin/env python3
r"""
Human-first reader edition of the CEG spec -> ceg-0.18-reader.pdf.
...
def prefilter(text, fname):
    ...
    if fname.startswith("01_"):
        text = re.sub(
            r"\n1\. \*\*PRIOR_ART_SCAN.*?(?=\n\*\*Future extensions)",
            "\n*Fifteen independent design exercises (consent, families, communities, "
...
if __name__ == "__main__":
    ...
    (D/"ceg-0.18-reader.tex").write_text("\n".join(body), encoding="utf-8")
    print(f"wrote ceg-0.18-reader.tex ({len(FILES)} files, §16 + RC1 register + changelog wall stripped)")
