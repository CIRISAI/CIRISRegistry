#!/usr/bin/env python3
r"""
Human-first reader edition of the CEG spec -> ceg-0.17-reader.pdf.
...
lineage) stays in the source tree and in pdf/ceg-0.14.pdf.
"""
import re
...
def prefilter(text, fname):
    ...
    if fname.startswith("01_"):
        text = re.sub(
            r"\n1\. \*\*PRIOR_ART_SCAN.*?(?=\n\*\*Future extensions)",
            "\n*Fourteen independent design exercises (consent, families, communities, "
...
(with full lineage) is the markdown source and \texttt{ceg-0.14.pdf}. Conformance is
...
if __name__ == "__main__":
    ...
    (D/"ceg-0.17-reader.tex").write_text("\n".join(body), encoding="utf-8")
    print(f"wrote ceg-0.17-reader.tex ({len(FILES)} files, §16 + RC1 register + changelog wall stripped)")
