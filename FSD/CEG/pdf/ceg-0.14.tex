...
\textbf{Version}: 0.14 (Public Working Draft)
...
\item \textbf{CEG-API-Version header}: \texttt{CEG-Version: <current spec major.minor>} on every response (track the README \texttt{Version:} field; currently \texttt{0.14}); clients SHOULD echo \texttt{CEG-Accept-Version: <pinned-version>} on request, naming the version they were built against. Per §0.3 SemVer policy, MAJOR mismatch is a wire-incompat reject; MINOR mismatch is compatible (clients MAY warn).
...
Per-chunk content sealing uses AES-256-GCM (NIST FIPS 197 + SP 800-38D). The 12-byte (96-bit) nonce follows the \textbf{STREAM} layout (Hoang, Reyhanitabar, Rogaway, Vizár — \textit{Online Authenticated-Encryption and its Nonce-Reuse Misuse-Resistance}, CRYPTO 2015):
...
\textbf{Forward secrecy = forward-only, NO PCS} (consistent with the §11.7.1 CEG 0.7 Option A discipline). MLS-style O(log N) rekey tree = \textbf{1.x, additive} (tree-position rides on the opaque \texttt{key\_grant} payload; no table migration).
