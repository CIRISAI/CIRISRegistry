...
\normalsize\mdseries Version 0.15 --- Reader Edition\\[2pt]
...
\textbf{Version}: 0.15 (Public Working Draft)

\textbf{Status}: Public Working Draft (2026-06-06). NOT a stable release. Wire-format primitives and namespace shape are settled in this draft except where 0.x → 0.(x+1) wire-breaks are noted; canonical-encoding details, response schemas, and registry process are scaffolded but not finalized. Implementers SHOULD pin against the 0.x series and expect breaking changes until 1.0 publication.


\begin{quote}\itshape \textbf{Human-reading edition:} \texttt{pdf/ceg-0.15-reader.pdf} is the de-editorialized, deduplicated, human-first cut (version history, per-path narrative, and provenance cross-refs stripped). This source tree remains the canonical working draft and retains that lineage.\end{quote}
...
Where framing produced a concrete wire consequence, that consequence is restated as a normative rule in its own section (e.g., structural invisibility is motivated informatively but enforced normatively at §10.1.4, bounded by §1.5). When in doubt, the RFC-2119 keywords and the §1.4 surface govern; informative prose never overrides them.
...
\textbf{Scope of the claim (read this before citing "1+4").} What follows is an \textbf{inductive adequacy result, not a closure theorem.} The fourteen paths below show the structural set is \textit{expressive across the surfaces tested}; they do \textbf{not} prove it generates \textit{every} expressible structured claim. We have not defined the class of structured claims and proven 1+4 generates exactly it — until someone does, "1+4 is adequate" means "adequate across the fourteen surfaces examined," nothing stronger. The refutation bar ("exhibit a claim that cannot be composed") is, honestly, near-unfalsifiable while \textit{composition itself} is unbounded — so absence of a counterexample is weak evidence, and these paths should be read as accumulating confidence, not as proof.
...
\textit{Fourteen independent design exercises (consent, families, communities, streaming, payments, DNS-free addressing, settlement, ...) each composed without a new structural primitive; the enumeration lives in the canonical working draft.}
...
\subsection*{§1.5 Adversary model \& privacy non-goals (normative)}
...
\subsubsection*{§1.5.1 What the structural-invisibility primitive (§10.1.4) buys}
...
\subsubsection*{§1.5.2 Non-goals — what omission does NOT buy}
...
\subsubsection*{§1.5.3 Adversary classes (and where each is / is not addressed)}
...
\begin{quote}\itshape \textbf{0.1 SCAFFOLD NOTE}: The contracts below use newline-delimited \texttt{key=value} encoding for readability. CEG 0.2+ will replace this with TupleHash128 ([FIPS-202]) using explicit domain-separation labels, fixing newline-injection and field-confusion attack surfaces identified in 0.1 cryptographic review. The 0.2 canonical-bytes redesign lands under Phase B per CIRISRegistry\#30 review. Producers building against 0.1 SHOULD treat these contracts as unstable.\end{quote}


\paragraph{\texttt{SkillImportManifest} canonical bytes (0.1 interim)}\mbox{}\\
...
canonical_bytes = sha256(
    ...
    "valid_until=" || optional_rfc3339_canonical_or_empty         // per S0.5
)
\end{lstlisting}

Hybrid signature: Ed25519 over \texttt{canonical\_bytes}; ML-DSA-65 over \texttt{canonical\_bytes || ed25519\_signature\_bytes} (bound payload).


\paragraph{Per-locale Merkle composition (0.1 interim)}\mbox{}\\
...
{\footnotesize
\begin{longtable}{|p{8.0cm}|p{8.0cm}|}\hline
\textbf{\texttt{wrap\_algorithm}} & \textbf{Algorithm} \\ \hline\endhead
\texttt{X25519AesGcmHkdfSha256} & HPKE RFC 9180 base-mode shape; X25519 KEM + HKDF-SHA-256 KDF + AES-256-GCM AEAD. \textbf{v1 default.} \\ \hline
\end{longtable}}
...
No new structural primitive, no new subject\_kind. Twelfth path (§1.4) — the wire format expresses \textbf{its own addressing layer} (DNS-free, self-certifying member resolution) by composition.
...
\textbf{No at-rest cascade}: community content emits \texttt{holds\_bytes:sha256:*} per status quo. Visibility scoping happens via consumer policy reading the community's roster + the Contribution's \texttt{community\_id} envelope field; there is no substrate-level DEK wrap. Reasons: communities can be LARGE (10K+ members); per-member DEK wrap on every emission would be operationally infeasible; the privacy property for communities is \textbf{cohort-filtered visibility}, not \textbf{byte-level invisibility} (that's for self/family).
...
Membership changes ride existing \texttt{supersedes}. \#\#\#\#\# Canonical \texttt{cohort\_subkind: infrastructure} (CEG 0.11 addition)
...
Withdrawal rides existing \texttt{withdraws}. Consent revocation composes via CEG 0.6 primitives. \#\#\#\# §5.6.8.12 \texttt{settlement} — CEG↔value-transfer linkage (CEG 0.14 addition)
...
**\textbf{ Fourteenth path (§1.4) — the wire format expresses }commerce-relationship auditability** (the receipt, not the rail) as composition, closing the last form of internet traffic from the completeness audit (CIRISRegistry\#59).
...
on_member_added(scope_target, new_member_key):
    ...
    AND C is still substrate-admitted (not withdrawn / not expired):
        emit key_grant {
            wrap_algorithm:     X25519AesGcmHkdfSha256,  // CEG 0.3 S5.6.8.4 default
...
Sibling to §8.1.12 Policy L (self/family) but with different defaults — community content federates (emits \texttt{holds\_bytes:sha256:*} per status quo); there is NO at-rest DEK cascade.
...
\paragraph{§8.1.13.3 No at-rest DEK cascade (the load-bearing difference vs Policy L)}\mbox{}\\


CEG 0.7 §8.1.12.4 Policy L cascade does NOT apply to community. Reasons:


\begin{itemize}\setlength\itemsep{0pt}
\item Communities can be LARGE (10K+ members)
\item Per-member DEK wrap on every emission would be operationally infeasible
\item The privacy property for communities is \textbf{cohort-filtered visibility} (consumer policy reads roster + envelope \texttt{community\_id}), NOT \textbf{byte-level invisibility} (that's for self/family)
\item Content emitted at \texttt{cohort\_scope: community} emits \texttt{holds\_bytes:sha256:*} per status quo; non-community peers see the directory entries but consumer policy filters them out
\end{itemize}


Consumer policy reading a \texttt{cohort\_scope: community} Contribution composes:
...
consumer_allows_visibility(C, viewer_key):
    ...
    return viewer_key in community
\end{lstlisting}

This is consumer-side; substrate emits per status quo.


\paragraph{§8.1.13.4 No automatic forward-secrecy decision (community has no removed-member-can-still-decrypt problem)}\mbox{}\\


CEG 0.7 §8.1.12.5 Option A doesn't apply because there's no DEK to retain. Removed community members lose visibility forward (consumer policy stops including their queries in the cohort filter) but had byte-level access during membership via the public \texttt{holds\_bytes:*} chain. No re-encryption ceremony needed; no key\_grant cascade to cancel.
...
\begin{quote}\itshape \textbf{0.1 SCAFFOLD NOTE}: The discriminator + nonce binding below addresses the cross-invocation-replay hole identified by CEG 0.1 cryptographic + red-team review. 0.2 may refine the encoding when §5.2.1 canonical-bytes redesigns to TupleHash128.\end{quote}
...
\item \textbf{CEG-API-Version header}: \texttt{CEG-Version: <current spec major.minor>} on every response (track the README \texttt{Version:} field; currently \texttt{0.15}); clients SHOULD echo \texttt{CEG-Accept-Version: <pinned-version>} on request, naming the version they were built against. Per §0.3 SemVer policy, MAJOR mismatch is a wire-incompat reject; MINOR mismatch is compatible (clients MAY warn).
...
\begin{quote}\itshape \textbf{Scope (normative — see §1.5):} structural invisibility buys \textbf{content-holding confidentiality only}. It does NOT hide that the relationship exists (\texttt{family\_id}, admission \texttt{hard\_case:*} events), who-talks-to-whom (resolution + RET announce/path-request expose endpoints), or traffic-analysis channels (STH cadence, key-cascade timing, chunk size/rate). It is not unobservability or metadata privacy — those require the opt-in Anonymous Tier. Do not represent CEG as providing the stronger properties.\end{quote}
...
On admission of a Contribution C with cohort_scope in {self, family}:
    ...
    substrate MUST wrap C's DEK via key_grant (S5.6.8.4) to:
...
\textbf{Composition with at-rest encryption flow} (CIRISPersist\#152): when content is admitted at \texttt{cohort\_scope: self}, persist wraps the DEK under each currently-admitted \texttt{identity\_occurrence}'s \texttt{occurrence\_key\_id}. When content is admitted at \texttt{cohort\_scope: family}, persist wraps under each \texttt{member.key\_id} in the named family's current roster. New occurrence / new family-member admission triggers retroactive \texttt{key\_grant} emission for all extant \texttt{cohort\_scope: self|family} content (the "I bought a new phone and want my Twitter history" / "I added Carol to the household" flows from §5.6.8.9 worked example).
...
\begin{itemize}\setlength\itemsep{0pt}
\item **\texttt{prefix[7]}** — derived, NOT transmitted: \texttt{prefix = HKDF-SHA256(epoch\_dek; info = "ciris-stream-nonce/v1" ‖ stream\_id ‖ epoch)[0..7]}. Matches the **\texttt{KEY\_GRANT\_V1\_INFO}** versioned-context HKDF pattern at \texttt{CIRISVerify/src/ciris-crypto/src/key\_grant.rs:71} (\texttt{b"cewp-key-grant/v1"}, used at \texttt{key\_grant.rs:163} in \texttt{kdf::hkdf\_sha256(shared\_secret, \&salt, KEY\_GRANT\_V1\_INFO, 32)}). Per-\texttt{(stream\_id, epoch)} unique; verifiable by any holder of the epoch DEK
...
Member removal & \textbf{MANDATORY rotation} — the forward-only-unsubscribe enforcement & Subsequent epochs sealed under a DEK the removed member doesn't have \\ \hline
...
\textbf{E2 — pull-only RC1} & RC1 multicast = \textbf{pull-only}: producer seals chunks under the epoch DEK → emits \texttt{holds\_bytes:sha256:*} → subscribers pull via the existing \texttt{ContentFetch} path. Relay / fan-out tree → 1.x (CIRISRegistry\#46 / \#43) \\ \hline
...
0.1-CRIT canonical-bytes newline-injection & \textbf{SCAFFOLDED} in §5.2.1 with 0.2 redesign committed (TupleHash128 + domain-separation labels) &  \\ \hline
...
Canonical-bytes redesign (TupleHash128 + domain-separation labels) & Phase B 0.2 commitment per CEG 0.1 review \\ \hline
...
Last updated: 2026-06-01 (CEG 0.9 Public Working Draft — \texttt{federation\_keys.identity\_type} generalized to a set of roles for single-key cohabitation per CIRISRegistry\#49 + CIRISAgent\#856; representation-only wire-break, ---
