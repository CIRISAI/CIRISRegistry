...
| Decision | Behavior |
|---|---|
| **E2 — pull-only RC1** | RC1 multicast = **pull-only**: producer seals chunks under the epoch DEK → emits `holds_bytes:sha256:*` → subscribers pull via the existing `ContentFetch` path. Relay / fan-out tree → 1.x (CIRISRegistry#46 / #43) |
